  ...
  return [
    ...
    mapping && (mapping.xField || mapping.yField || mapping.groupField)
      ? `Field mapping: x=${mapping.xField || "auto"}, y=${
          mapping.yField || "auto"
        }, group=${mapping.groupField || "none"}, agg=${
          mapping.agg || "mean"
        }, error=${mapping.errorMetric || "none"}, multi=${
          mapping.multiCharts ? "true" : "false"
        }`
      : "",
    `User goal: ${safeGoal}`,
...
function detectDelimiter(lineSamples) {
  const testLine = Array.isArray(lineSamples)
    ? lineSamples.join("\n")
    : lineSamples || "";
  if (testLine.includes("\t")) return "\t";
  if (
    (testLine.match(/,/g) || []).length >= (testLine.match(/;/g) || []).length
  )
    return ",";
  return ";";
}
...
function parseTabularContent(rawText) {
  ...
  for (let i = 1; i < lines.length && rows.length < MAX_ROWS_TO_STORE; i += 1) {
  ...
  const numericHints = headers
    .map((header) => {
      ...
      const numericCount = values.filter((value) =>
        isProbablyNumeric(value)
      ).length;
      const ratio =
        sampleRows.length > 0 ? numericCount / sampleRows.length : 0;
  ...
  const categoryHints = headers
    ...
    .flatMap(({ idx }) =>
      (valueStats[headers[idx]]?.top_values || []).map((entry) => entry.value)
    )
...
function normalizeEchartsOption(option, extraHints = {}) {
    ...
    if (extraHints.letters && typeof extraHints.letters === "object") {
      Object.keys(extraHints.letters).forEach((key) => {
        if (key !== undefined && key !== null)
          manualCategorySet.add(String(key));
      });
    }
    ...
    if (Array.isArray(extraHints.categoryHints)) {
      extraHints.categoryHints.forEach((name) => {
        if (name !== undefined && name !== null)
          manualCategorySet.add(String(name));
      });
    }
  }
  ...
  sanitized.series = seriesArray.map((series, idx) => {
    ...
    if (resolvedType === "heatmap") {
      ...
      if (!nextSeries.label) {
        ...
        nextSeries.label = {
          ...nextSeries.label,
          show: normalizedHeatmap.data.length <= 250,
        };
      }
      nextSeries.label = {
        ...nextSeries.label,
      ...
      if (normalizedHeatmap.valueRange) {
        heatmapHints.min = Math.min(
          heatmapHints.min,
          normalizedHeatmap.valueRange.min
        );
        heatmapHints.max = Math.max(
          heatmapHints.max,
          normalizedHeatmap.valueRange.max
        );
      }
    }
    return nextSeries;
  ...
  const manualCategories =
    manualCategorySet.size > 0 ? Array.from(manualCategorySet) : null;
  if (heatmapHints.xCategories) {
    sanitized.xAxis = ensureCategoryAxis(
      sanitized.xAxis,
      heatmapHints.xCategories
    );
  } else if (manualCategories) {
    sanitized.xAxis = ensureCategoryAxis(sanitized.xAxis, manualCategories, {
      merge: true,
    });
  }
  if (heatmapHints.yCategories) {
    sanitized.yAxis = ensureCategoryAxis(
      sanitized.yAxis,
      heatmapHints.yCategories
    );
  }
  if (
  ...
  if (Array.isArray(sanitized.series)) {
    sanitized.series = sanitized.series.filter((s) => {
      const n =
        s && (s.name || s.label) ? String(s.name || s.label).toLowerCase() : "";
...
function convertChartConfigToEcharts(chartConfig) {
  ...
  const labels = Array.isArray(data.labels)
    ? data.labels
    : Array.from(
        { length: datasets[0]?.data?.length || 0 },
        (_, idx) => `Item ${idx + 1}`
      );
  const resolvedType = pickSeriesType(
...
function pickSeriesType(candidate, fallback = "line") {
  const normalized =
    typeof candidate === "string" ? candidate.toLowerCase().trim() : "";
  ...
  dataArray.forEach((entry) => {
    ...
    if (Array.isArray(entry) && entry.length >= 3) {
      ...
      yVal =
        ...
        entry.l;
      metric =
        entry.value ?? entry.v ?? entry.score ?? entry.z ?? entry.correlation;
    } else {
      return;
    }
...
function ensureCategoryAxis(axisOption, categories, { merge = false } = {}) {
  ...
  if (Array.isArray(axisOption)) {
    const next =
      axisOption.length > 0 ? axisOption.map((axis) => ({ ...axis })) : [{}];
    if (!next[0]) {
  ...
  const axis =
    axisOption && typeof axisOption === "object"
      ? { ...axisOption }
      : { type: "category" };
...
function mergeAxisCategories(existing, additions) {
  const union = Array.isArray(existing)
    ? existing.map((item) => (item != null ? String(item) : item))
    : [];
...
function attachErrorBars(option, errorBars) {
  try {
    if (!option || !Array.isArray(errorBars) || errorBars.length === 0)
      return option;
    ...
    const isBar =
      ...
      option.series[0] &&
      String(option.series[0].type).toLowerCase() === "bar";
    const isVertical =
      isBar &&
      ...
      renderItem: function (params, api) {
        ...
        return {
          ...
          children: [
            {
              type: "line",
              shape: { x1: xCoord, y1: lowCoord, x2: xCoord, y2: highCoord },
              style,
            },
            {
              type: "line",
...
export default function AiDataVisualizerPage() {
  ...
  const table = useMemo(
    () => parsedTableOverride || parseTabularContent(fileText),
    [parsedTableOverride, fileText]
  );
  const datasetSummary = useMemo(
    () => buildDatasetSummary(fileInfo, table),
    [fileInfo, table]
  );
        ...
        reader.onload = (e) => {
          try {
            ...
            names.forEach((nm) => {
              ...
              const headers = (rowsA1[0] || []).map(
                (h, idx) => (h && String(h).trim()) || `Column ${idx + 1}`
              );
              const body = rowsA1.slice(1, MAX_ROWS_TO_STORE + 1).map((arr) => {
                const r = {};
                headers.forEach((h, i) => {
                  r[h] = arr[i] != null ? String(arr[i]) : "";
                });
                return r;
              });
  ...
  async function handleAnalyze() {
    ...
    try {
      if (aiConfig.provider === "demo") {
        ...
        setRawAiText(
          JSON.stringify(
            { mode: "local-deterministic", chart_option: localResult.option },
            null,
            2
          )
        );
      ...
      const result = await requestAI(
        ...
        { question: prompt },
        {
          ...
          systemPrompt:
            "Return only a strict JSON object containing summary, insights, and chart_option.",
        }
      );
      ...
      if (fallback?.option) {
        ...
        setRawAiText(
          JSON.stringify(
            { mode: "local-fallback", chart_option: fallback.option },
            null,
            2
          )
        );
        setStatusMessage(
          `The AI request failed; a chart computed from the uploaded rows is shown instead. ${
            err?.message || ""
          }`.trim(),
          "warning"
        );
  ...
  async function processAiText(aiText) {
    ...
    if (!parsed) {
      ...
      if (fallback && fallback.option) {
        ...
        setRawAiText(
          JSON.stringify(
            { mode: "offline", chart_option: fallback.option },
            null,
            2
          )
        );
  ...
  function buildTableFromRows(headers, rows) {
    const storedRows = rows.slice(0, MAX_ROWS_TO_STORE);
    const sampleRows = storedRows.slice(0, MAX_SAMPLE_ROWS);
    const valueStats = headers.reduce((acc, h) => {
      const map = new Map();
      storedRows.forEach((r) => {
        const v = r[h];
        if (v && String(v).length > 0) map.set(v, (map.get(v) || 0) + 1);
      });
      const top = Array.from(map.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([value, count]) => ({ value, count }));
      acc[h] = { unique_count: map.size, top_values: top };
      return acc;
    }, {});
    const numericHints = headers.filter((h) => {
      const vals = sampleRows.map((r) => r[h]);
      const cnt = vals.filter((v) => isProbablyNumeric(v)).length;
      const ratio = sampleRows.length > 0 ? cnt / sampleRows.length : 0;
      return ratio >= 0.6;
    });
    const categoryHints = headers
      .filter((h) => !numericHints.includes(h))
      .flatMap((h) => (valueStats[h]?.top_values || []).map((e) => e.value))
  ...
  function buildTableFromJson(json) {
    if (Array.isArray(json)) {
      ...
      const rows = json.slice(0, MAX_ROWS_TO_STORE).map((obj) => {
        const r = {};
        headers.forEach((h) => {
          const v = obj && obj[h] != null ? obj[h] : "";
          r[h] = String(v);
        });
        return r;
      });
    ...
    if (json && typeof json === "object") {
      if (Array.isArray(json.data) && Array.isArray(json.columns)) {
        const headers = json.columns.map(
          (h, idx) => (h && String(h).trim()) || `Column ${idx + 1}`
        );
        const rows = json.data.slice(0, MAX_ROWS_TO_STORE).map((arr) => {
          const r = {};
          headers.forEach((h, i) => {
            r[h] = arr[i] != null ? String(arr[i]) : "";
          });
      ...
      if (Array.isArray(json.rows) && Array.isArray(json.headers)) {
        const headers = json.headers.map(
          (h, idx) => (h && String(h).trim()) || `Column ${idx + 1}`
        );
        const rows = json.rows.slice(0, MAX_ROWS_TO_STORE).map((obj) => {
          const r = {};
          headers.forEach((h) => {
            r[h] = obj && obj[h] != null ? String(obj[h]) : "";
          });
          return r;
        });
        return buildTableFromRows(headers, rows);
  ...
  return (
    <AppScaffold appId="ai-data-visualizer">
            ...
            <section className={styles.card}>
                ...
                <label className={styles.fieldLabel}>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedSheet(nm);
                      setParsedTableOverride(excelTables[nm] || null);
                    }}
                  >
                    {excelSheets.map((nm) => (
              ...
              {status?.text && (
                <div
                  className={clsx(
                    styles.statusBadge,
                    styles[`status--${status.tone || "info"}`]
                  )}
                    ...
                    onClick={() => {
                      ...
                      if (res && res.option) {
                        ...
                        setRawAiText(
                          JSON.stringify(
                            {
                              mode: "offline-mapping",
                              chart_option: res.option,
                            },
                            null,
                            2
                          )
...
function buildOfflineVisualization(tableObj, goalText, mapping = {}) {
  if (
    ...
    (tableObj.rows || []).length === 0
  )
    return null;
  const keywords = extractGoalKeywords(goalText || "");
  const numericCols = tableObj.headers.filter((h) =>
    tableObj.numericHints.includes(h)
  );
  const categoryCols = tableObj.headers.filter(
    (h) => !tableObj.numericHints.includes(h)
  );
  const {
    xField,
    yField,
  ...
  if (xField && yField) {
    ...
    const opt = buildGroupedBarOption(
      grp.labels,
      grp.series,
      xField,
      `${agg}(${yField})`
    ...
    if (multiCharts) {
      const gstats = computeGroupedValues(
        tableObj.rows,
        xField,
        yField,
        groupField
      );
      const opt2 = buildGroupedViolinOption(
        gstats.labelsX,
        gstats.groups,
        gstats.valuesByGroup,
        yField
      );
      return {
        option: composeDualOption(opt, opt2),
  ...
  if (keywords.includes("correlation") && numericCols.length >= 2) {
    ...
    const opt = buildCorrelationHeatmapOption(
      corr.labelsX,
      corr.labelsY,
      corr.matrix
    );
  ...
  if (
    ...
    if (groupField) {
      ...
      const left = buildGroupedBoxplotCustomOption(
        gstats.labelsX,
        gstats.groups,
        gstats.valuesByGroup,
        y
      );
      const right = buildGroupedViolinOption(
        gstats.labelsX,
        gstats.groups,
        gstats.valuesByGroup,
        y
...
function extractGoalKeywords(text) {
  ...
  map.forEach(([key, re]) => {
    if (new RegExp(re).test(t)) keys.push(key);
  });
  return keys;
}
...
function aggregateByXYGroup(rows, xField, yField, gField, method) {
  ...
  rows.forEach((r) => {
    ...
    if (!xMap.has(x)) {
      xMap.set(x, xCats.length);
      xCats.push(x);
      series.forEach((item) => item.values.push([]));
    }
    if (!gMap.has(g)) {
      gMap.set(g, gCats.length);
      gCats.push(g);
      series.push({
        name: g,
        values: Array.from({ length: xCats.length }, () => []),
      });
    }
    const xi = xMap.get(x);
    const gi = gMap.get(g);
  ...
  const finalSeries = series.map((s) => ({
    name: s.name,
    data: s.values.map((arr) => aggregate(arr, method)),
  }));
...
function computeErrorBars(rows, xField, yField, metric) {
  ...
  Array.from(map.keys()).forEach((k) => {
    ...
    const sd = Math.sqrt(
      arr.reduce((s, v) => s + (v - mean) * (v - mean), 0) / Math.max(1, n - 1)
    );
    const se = sd / Math.sqrt(n);
    if (metric === "stddev")
      bars.push({ name: k, low: mean - sd, high: mean + sd });
    else if (metric === "se")
      bars.push({ name: k, low: mean - se, high: mean + se });
  });
  return bars;
}
...
function computeCorrelationMatrix(rows, fields) {
  ...
  const corr = Array.from({ length: n }, () =>
...
function buildScatterOptionFromRows(rows, xField, yField, groupField) {
  ...
  const series = Object.keys(points).map((k) => ({
    name: k,
    type: "scatter",
    data: points[k],
  }));
...
function buildGroupedViolinOption(labelsX, groups, valuesByGroup, yName) {
  ...
  const series = groups.map((g, gi) => ({
    ...
    renderItem: function (params, api) {
      ...
      const maxD = Math.max.apply(null, dens) || 1;
      const scale =
        typeof api.size === "function" ? api.size([1, 0])[0] * 0.12 : 10;
      const leftPts = [];
      const rightPts = [];
      for (let i = 0; i < ys.length; i += 1) {
        const w = (dens[i] / maxD) * scale;
      ...
      const points = leftPts.concat(rightPts.reverse());
      const style = api.style({
        fill: "rgba(79,129,189,0.25)",
        stroke: "var(--ifm-color-emphasis-800)",
      });
      return { type: "polygon", shape: { points }, style };
    },
    data: labelsX.map((lab, xi) => ({
      xIdx: xi,
      values: valuesByGroup[g][xi] || [],
    })),
  }));
  return normalizeEchartsOption({
...
function computeBandwidth(arr) {
  ...
  return h || (Math.max(...arr) - Math.min(...arr)) / 20 || 1;
}

function kdePoint(arr, y, h) {
...
function composeDualOption(opt1, opt2) {
  ...
  const x1 = Array.isArray(a1.xAxis)
    ? a1.xAxis[0]
    : a1.xAxis || { type: "category" };
  const y1 = Array.isArray(a1.yAxis)
    ? a1.yAxis[0]
    : a1.yAxis || { type: "value" };
  const x2 = Array.isArray(a2.xAxis)
    ? a2.xAxis[0]
    : a2.xAxis || { type: "category" };
  const y2 = Array.isArray(a2.yAxis)
    ? a2.yAxis[0]
    : a2.yAxis || { type: "value" };
  const series1 = Array.isArray(a1.series)
    ? a1.series.map((s) => ({ ...s, xAxisIndex: 0, yAxisIndex: 0 }))
    : [];
  const series2 = Array.isArray(a2.series)
    ? a2.series.map((s) => ({ ...s, xAxisIndex: 1, yAxisIndex: 1 }))
    : [];
  return {
...
function computeGroupedValues(rows, xField, yField, gField) {
  ...
  rows.forEach((r) => {
    ...
    if (!xMap.has(x)) {
      xMap.set(x, xCats.length);
      xCats.push(x);
      gCats.forEach((group) => valuesByGroup[group].push([]));
    }
    if (!gMap.has(g)) {
      gMap.set(g, gCats.length);
      valuesByGroup[g] = Array.from({ length: xCats.length }, () => []);
      gCats.push(g);
    }
    const xi = xMap.get(x);
...
function buildGroupedBoxplotCustomOption(
  labelsX,
  groups,
  valuesByGroup,
  yName
  ...
  const series = groups.map((g, gi) => ({
    ...
    renderItem: function (params, api) {
      ...
      const min = d.stats[0],
        ...
        q3 = d.stats[3],
        max = d.stats[4];
      const pMin = api.coord([xs, min]);
      const pQ1 = api.coord([xs, q1]);
      const pMed = api.coord([xs, med]);
      const pQ3 = api.coord([xs, q3]);
      const pMax = api.coord([xs, max]);
      if (!pMin || !pQ1 || !pMed || !pQ3 || !pMax) return null;
      const boxW =
        typeof api.size === "function" ? api.size([1, 0])[0] * 0.18 : 12;
      const style = api.style({
      ...
      return {
        ...
        children: [
          {
            type: "line",
            shape: { x1: pMin[0], y1: pMin[1], x2: pQ1[0], y2: pQ1[1] },
            style,
          },
          {
            type: "line",
            shape: { x1: pQ3[0], y1: pQ3[1], x2: pMax[0], y2: pMax[1] },
            style,
          },
          {
            type: "line",
    ...
    data: labelsX.map((lab, xi) => ({
      xIdx: xi,
      stats: fiveNumber(valuesByGroup[g][xi]),
    })),
  }));
  return normalizeEchartsOption({
