...
export default function Root({children}) {
  ...
  const QR_URL = `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(WECHAT_URL)}`;

  return (
    <>
      {children}
      <div className="wechat-float-container">
        {/* QR code popover — appears above the button on hover */}
        <div className="wechat-float-qr">
          <img
            src={QR_URL}
          ...
          <p className="wechat-float-qr-hint">
            {isChinese ? '微信扫码发起咨询' : 'Scan to chat on WeChat'}
          </p>
          <Link
